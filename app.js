let playerScore = 0;
let computerScore = 0;
let numberOfRoundWins = 5;

function updateScore() {
    document.getElementById('player-score').textContent = playerScore;
    document.getElementById('computer-score').textContent = computerScore;
    if (playerScore >= numberOfRoundWins || computerScore >= numberOfRoundWins) {
        endGame();
    }
}
...
function addChoiceButtonsEvents() {
    ...
    choiceButtons.forEach(button => button.addEventListener('click',
        choiceButtonsEventListenerFunction));
}

function removeChoiceButtonsEvents() {
    const choiceButtons = Array.from(document.getElementsByClassName('choice-button'));
    choiceButtons.forEach(button => button.removeEventListener('click',
        choiceButtonsEventListenerFunction));
}
...
function play() {
    playerScore = 0;
    computerScore = 0;
    numberOfRoundWins = 5;  // hardcoded number of round to play
    updateSubtitle(`First to win ${numberOfRoundWins} rounds, wins the game!`);
